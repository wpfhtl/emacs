...
(defun update-matlab-mode-install-path ()
  "Automatically update the matlab-mode-install-path by the results
of 'path' in matlab-shell-mode"
  (if (matlab-shell-active-p)
      (let ((output (matlab-shell-collect-command-output "path"))
            (lp))
        (setq output (substring-no-properties output 15))
        (while (string-match "[^ \t]\\(.*\\)$" output)
          (let* ((b (match-beginning 0))
                 (e (match-end 0))
                 (path (substring-no-properties output b e)))
            (if (file-directory-p path)
                (if (string-match "\\(.*\\)[/\\]$" path)
                    (setq lp (append lp (list path)))
                  (setq lp (append lp (list (concat path
                                                    (if (eq 'window-system 'w32)
                                                        "\\"
                                                      "/")
                                                    ))))))
            (setq output (substring-no-properties output (1+ e)))))
        (setq matlab-mode-install-path lp))))
...
`matlab-mode-install-path' if you would like to have them included."
  (update-matlab-mode-install-path)
  (interactive
   (list
